...
def _retrieval_engine_authority_state(result: Any) -> str:
    ...
    if status in {"denied", "blocked"} or _retrieval_engine_result_has_scope_failure(result):
        return "fail_closed"
    return "diagnostics"
...
def _retrieval_engine_result_has_scope_failure(result: Any) -> bool:
    for diagnostic in getattr(result, "diagnostics", ()) or ():
        ...
        failed_gates = details.get("failed_gates", ()) if isinstance(details, dict) else ()
        if code in {"source_scope_violation", "source_identity_mismatch"}:
            return True
        if any(str(gate) in {"source_scope", "source_identity"} for gate in failed_gates):
            return True
    return False
...
def _retrieval_engine_anchor_has_exact_geometry(source_anchor: dict[str, Any]) -> bool:
    ...
    if anchor.get("sheet") not in (None, "") and (
        anchor.get("range") not in (None, "")
        or anchor.get("cell") not in (None, "")
    ):
        return True
...
def _retrieval_engine_context_budget(result: Any) -> dict[str, Any]:
    ...
    omitted_accepted_bundle_count = int(
        budget.get("omitted_accepted_bundles") or 0
    )
    context_budget = {
        ...
        "reference_count": int(
            budget.get("references")
            or len(getattr(result, "references", ()) or ())
        ),
...
def _retrieval_engine_contract_diagnostics(
    ...
    status = str(getattr(result, "status", "") or "").strip().lower()
    terminal_reason = (
        str(getattr(result, "terminal_reason", "") or "").strip() or status or "unknown"
    )
    authority_state = _retrieval_engine_authority_state(result)
    runtime_mode = _retrieval_engine_runtime_mode(authority_state)
    for diagnostic in [
        ...
        *(getattr(result, "diagnostics", ()) or ()),
        ...
        if status in {"no_evidence", "blocked", "denied", "timeout", "malformed", "error"}:
