(() => {
  ...
  const configuredLogoWidth = Math.max(24, toFiniteNumber(config.launcherWidth || config.logoWidth, 68));
  const configuredLogoHeight = Math.max(24, toFiniteNumber(config.launcherHeight || config.logoHeight, 68));
  ...
  const rawLauncherBoxSize = Math.min(configuredLogoWidth, configuredLogoHeight);
  const fallbackLogoWidth = useRawLauncherImage ? rawLauncherBoxSize : configuredLogoWidth;
  const fallbackLogoHeight = useRawLauncherImage ? rawLauncherBoxSize : configuredLogoHeight;
  ...
  const rawLauncherImageScaleValue = toFiniteNumber(config.launcherImageScale ?? config.rawImageScale, 1);
  const rawLauncherImageScale = useRawLauncherImage
    ? Math.min(Math.max(rawLauncherImageScaleValue, 0.4), 1)
    : 1;
  const rawLauncherImageSize = useRawLauncherImage
    ? Math.round(rawLauncherBoxSize * rawLauncherImageScale)
    : 0;
  const launcherWidthCss = useRawLauncherImage
    ? `${fallbackLogoWidth}px`
    : `clamp(${responsiveLauncherMinWidth}px, 15vw, ${fallbackLogoWidth}px)`;
  const launcherHeightCss = useRawLauncherImage
    ? `${fallbackLogoHeight}px`
    : `clamp(${responsiveLauncherMinHeight}px, ${15 * launcherHeightRatio}vw, ${fallbackLogoHeight}px)`;
  ...
  const bubbleDuration = Math.max(0, toFiniteNumber(config.bubbleDuration, 5000));
    ...
    style.textContent = `
      ...
      *, *::before, *::after {
      ...
      .cpecc-launcher {
        ...
        z-index: 20;
        display: flex;
        width: ${launcherWidthCss};
        height: ${launcherHeightCss};
        align-items: center;
      ...
      .cpecc-launcher.is-image-raw .cpecc-logo-disc {
        width: ${rawLauncherImageSize}px;
        height: ${rawLauncherImageSize}px;
        border-radius: 0;
        background: transparent;
      }
